...
class LookaheadGenerator(object):
    def __init__(
        self, orig_gen: Iterator, max_lookahead: int = None, get_timeout: float = 10
    ):
        """
        Forked from: https://github.com/multiprocessing-generator/multiprocessing_generator
        ...
        self.ppid = None  # pid of the parent thread
        self.thread = Thread(target=wrapped)
        self.thread_started = False

    def finish_if_possible(self):
        """
        We can only terminate the child thread from the parent thread
        """
        if self.ppid == os.getpid() and self.thread:  # and self.thread.is_alive():
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        Kills the thread
        """
        assert self.thread_started and self.ppid == None or self.ppid == os.getpid()
