...
from gluonts.torch.util import copy_parameters, IterableDataset
...
from pts.model.tft.tft_network import (
    TemporalFusionTransformerNetwork,
    TemporalFusionTransformerPredictionNetwork,
)
from pts.model.tft.tft_transform import BroadcastTo, TFTInstanceSplitter
from torch.utils.data import Dataset, DataLoader

from ml_challenge.gluonts.model.tft.lightning_module import TemporalFusionTransformerLightningModule
...
class TemporalFusionTransformerEstimator(PyTorchLightningEstimator):
    ...
    def create_transformation(self) -> Transformation:
        transforms = (
            ...
            + [
                AsNumpyArray(field=name, expected_ndim=1)
                for name in chain(
                    self.static_feature_dims.keys(), self.dynamic_cardinalities.keys(),
                )
            ]
            + [
        ...
        if self.dynamic_cardinalities:
            ...
            transforms.extend(
                [
                    SetField(output_field=FieldName.FEAT_DYNAMIC_CAT, value=[[0]],),
                    AsNumpyArray(
                        field=FieldName.FEAT_DYNAMIC_CAT,
                        expected_ndim=2,
                        dtype=np.long,
        ...
        transforms.append(
            VstackFeatures(
                input_fields=input_fields, output_field=FieldName.FEAT_DYNAMIC_REAL,
            )
        )
        ...
        if self.past_dynamic_cardinalities:
            ...
            transforms.extend(
                [
                    SetField(
                        output_field=FieldName.PAST_FEAT_DYNAMIC + "_cat", value=[[0]],
                    ),
                    AsNumpyArray(
                        field=FieldName.PAST_FEAT_DYNAMIC + "_cat",
                        expected_ndim=2,
        ...
        if self.past_dynamic_feature_dims:
            ...
            transforms.extend(
                [
                    SetField(
                        output_field=FieldName.PAST_FEAT_DYNAMIC_REAL, value=[[0.0]],
                    ),
                    AsNumpyArray(
    ...
    def create_validation_data_loader(
        self, data: Dataset, module: TemporalFusionTransformerLightningModule, **kwargs,
    ) -> Iterable:
        ...
        return DataLoader(
            IterableDataset(validation_instances), batch_size=self.batch_size, **kwargs,
        )

    def create_lightning_module(self) -> TemporalFusionTransformerLightningModule:
    ...
    def create_predictor(
        self, transformation: Transformation, module: TemporalFusionTransformerLightningModule,
    ) -> PyTorchPredictor:
        ...
        copy_parameters(module.model, prediction_network)
