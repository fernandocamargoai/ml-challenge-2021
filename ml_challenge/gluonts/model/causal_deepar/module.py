...
class NOPScaler(nn.Module):
    ...
    def forward(
        ...
        scale = torch.ones_like(data).mean(
            dim=self.dim,
            keepdim=True,  # TODO: fill an issue about the lack of keepdim=True
...
class CausalDeepARModel(nn.Module):
    ...
    def __init__(
        ...
        self.param_proj = distr_output.get_args_proj(hidden_size + 1)
        self.target_shape = distr_output.event_shape
        self.control_output = control_output
        self.control_param_proj = control_output.get_args_proj(hidden_size)
        self.control_target_shape = control_output.event_shape
    ...
    def unroll_lagged_rnn(
        ...
        control_context = past_control[:, -self.context_length :]
        observed_context = past_observed_values[:, -self.context_length :]
        _, scale = self.scaler(context, observed_context)
        _, control_scale = self.control_scaler(control_context, observed_context)

        prior_target_input = past_target[:, : -self.context_length] / scale
        prior_control_input = past_control[:, : -self.context_length] / control_scale
        prior_input = torch.cat(
            (prior_target_input.unsqueeze(-1), prior_control_input.unsqueeze(-1)),
            dim=-1,
        )
        target_input = (
        ...
        control_input = (
            torch.cat((control_context, future_control[:, :-1]), dim=1) / control_scale
            if future_control is not None
            else control_context / control_scale
        )
        input = torch.cat(
            (target_input.unsqueeze(-1), control_input.unsqueeze(-1)), dim=-1
        )
        ...
        static_feat = torch.cat(
            (embedded_cat, feat_static_real, scale.log(), control_scale.log()), dim=1,
        )
        expanded_static_feat = static_feat.unsqueeze(1).expand(-1, unroll_length, -1)
        ...
        return output, new_state, scale, control_scale, static_feat
    ...
    def control_distribution(
        ...
        if trailing_n is not None:
            sliced_params = [p[:, -trailing_n:] for p in params]
        return self.control_output.distribution(sliced_params, scale=scale)
    ...
    def distribution(
        ...
        rnn_outputs, _, scale, control_scale, _ = self.unroll_lagged_rnn(
        ...
        control_params = self.control_param_proj(rnn_outputs)

        control = torch.cat(
            (
                past_control[:, self.history_length - self.context_length :, ...],
                future_control[:, :-1],  # Why?
            ),
            dim=1,
        )
        ...
        return (
            self.control_distribution(control_params, scale=control_scale),
            self.output_distribution(params, scale=scale),
        )
    ...
    def forward(
        ...
        rnn_outputs, state, scale, control_scale, static_feat = self.unroll_lagged_rnn(
            feat_static_cat=feat_static_cat,
            feat_static_real=feat_static_real,
            past_time_feat=past_time_feat,
            past_target=past_target,
        ...
        control_dist_args = self.control_param_proj(rnn_outputs)
        ...
        for k in range(self.prediction_length - 1):
...
class LaggedLSTM(nn.Module):
    ...
    def get_lagged_subsequences(
        self, sequence: torch.Tensor, subsequences_length: int,
    ) -> torch.Tensor:
        """
    ...
    def forward(
        ...
        lagged_sequence = self.get_lagged_subsequences(
            sequence=sequence, subsequences_length=input.shape[1],
        )
